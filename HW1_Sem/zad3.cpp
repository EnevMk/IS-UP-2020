...
int main() {
    ...
    while (eval_level(trudnost) == Difficulty::INVALID) {
        std::cout << "Invalid input. Try again\n";
        std::cin >> trudnost;

    }

    const int num_of_states = limit(eval_level(trudnost));
    const int sleep_time = wait_time(eval_level(trudnost));

    std::string states[] = {"up", "down", "left", "right", "dog", "cat", "banana", "apple"};

    std::cout << "\nStarting " << trudnost << " mode. You will have " << num_of_states << " states (";
